 ...
 */
function matchWord(text, cur, preword) {
 ...
 * @return {array}
 */
function filterWord(words, preword, text) {
    console.time('filterWord');
    let POSTAG = exports.segment.POSTAG;
    let TABLE = exports.segment.getDict('TABLE');
    let ret = [];
    // 将单词按位置分组
 ...
 * @return {object}
 */
function getPosInfo(words, text) {
    let wordpos = {};
    // 将单词按位置分组
    for (let i = 0, word; word = words[i]; i++) {
        if (!wordpos[word.c]) {
 ...
 * @return {array}
 */
function getChunks(wordpos, pos, text, total_count = 0) {
    total_count++;
    ...
    // throw new Error();
    let ret = [];
    //console.log(total_count, words.length);
