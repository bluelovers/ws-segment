...
class DictTokenizer extends mod_1.SubSModuleTokenizer {
        ...
        for (let i = 0, chunk; chunk = chunks[i]; i++) {
            ...
            for (let j = 0, w; w = chunk[j]; j++) {
                if (w.w in TABLE) {
                    w.p = TABLE[w.w].p;
                    assess[i].a += w.f; // 总词频
                    if (j === 0 && isUnset_1.default(preword) && (w.p & POSTAG.D_V)) {
                        /**
                         * 將第一個字也計算進去是否包含動詞
                         */
                        has_D_V = true;
                    ...
                    if (prew) {
                        ...
                        if (nextw) {
                            ...
                            if (_temp_ok && w.p & POSTAG.D_P) {
                                if (index_1.hexAndAny(prew.p, POSTAG.D_N) && index_1.hexAndAny(nextw.p, POSTAG.D_N, POSTAG.D_V)) {
                                    assess[i].d++;
                                    _temp_ok = false;
                                }
                                else if (index_1.hexAndAny(prew.p, POSTAG.D_R) && index_1.hexAndAny(nextw.p, POSTAG.D_R)) {
                                    assess[i].d += 0.5;
                                    _temp_ok = false;
                                }
                            }
    ...
    getChunks(wordpos, pos, text, total_count = 0, MAX_CHUNK_COUNT) {
         ...
         */
        let m;
        if (!isUnset_1.default(m = text.match(/^((.+)\2{5,})/))) {
