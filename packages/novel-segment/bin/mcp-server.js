    ...
    try {
        ...
        switch (name) {
            case 'segment_text':
                options = {
                    text: args.text,
                    expectedFull: args.expectedFull,
                    expectedContains: args.expectedContains,
                    expectedContainsNot: args.expectedContainsNot,
                    expectedIndexOf: args.expectedIndexOf,
                    expectedIndexOfNot: args.expectedIndexOfNot,
                    dictEntries: args.dictEntries,
                    synonymEntries: args.synonymEntries,
                    blacklistWords: args.blacklistWords,
                    debugEach: args.debugEach,
                    outputFormat: 'json',
                };
                break;
            case 'segment_file':
                options = {
                    file: args.file,
                    expectedFullFile: args.expectedFullFile,
                    outputFile: args.outputFile,
                    outputFormat: args.outputFormat || 'json',
                };
                break;
            default:
                throw new types_js_1.McpError(types_js_1.ErrorCode.MethodNotFound, `未知的工具 / Unknown tool: ${name}`);
        }
        // 執行分詞測試
        // Execute segmentation test
        const result = await (0, cli_core_1.demoSegmentTestCore)(options);
        return {
            content: [
                {
                    type: 'text',
        ...
        throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `執行錯誤 / Execution error: ${error instanceof Error ? error.message : String(error)}`);
    }
});
// =============================================================================
// 啟動伺服器 / Start Server
