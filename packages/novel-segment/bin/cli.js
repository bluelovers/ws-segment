...
function loadConfigFile(configPath) {
    const absolutePath = (0, upath2_1.resolve)(configPath);
    if (!(0, fs_extra_1.existsSync)(absolutePath)) {
        console.error(`錯誤: 設定檔不存在 / Error: Config file does not exist: ${absolutePath}`);
        process.exit(1);
    }
    const ext = (0, upath2_1.extname)(absolutePath).toLowerCase();
    try {
        if (ext === '.json') {
            // 讀取 JSON 檔案
            // Read JSON file
            const content = (0, fs_extra_1.readFileSync)(absolutePath, 'utf8');
 ...
 */
function mergeOptions(fileOptions, cliOptions) {
    // 命令列參數優先於設定檔
