...
function lazyMatch(a, b, options = {}) {
  ...
  !bool && assert.fail(`expected ${inspectFn(a)} to have includes ordered members ${inspectFn(b)}`);
  return bool;
}
...
function lazyMatchSynonym001(a, b_arr, options = {}) {
  ...
  bool = b_arr.every(function (bb) {
    ...
    if (j > -1 && j >= i) {
      ...
      assert.fail(`expected ${inspectFn(a)} to have have ${inspectFn(bb)} on index > ${i}, but got ${j}`);
    }
  });
  if (i === -1) {
    bool = false;
  }
  !bool && assert.fail(`expected ${inspectFn(a)} to have index of ordered members in ${inspectFn(b_arr)}`);
}
function lazyMatchSynonym001Not(a, b_arr, options = {}) {
  ...
  b_arr.every(function (bb) {
    ...
    if (j > -1 && j > i) {
      assert.fail(`expected ${inspectFn(a)} to not have ${inspectFn(bb)} on index > ${i}, but got ${j}`);
      return true;
    } else {
      i++;
    }
...
function lazyMatchNot(a, b, options = {}) {
  ...
  !bool && assert.fail(`expected ${inspectFn(a)} should not have includes ordered members ${inspectFn(b)}`);
  return bool;
}
