...
export class TableDictSynonymPanGu extends AbstractTableDictCore<string>
{
	...
	add(data: [string, string] & string[], skipExists?: boolean)
	{
		...
		// 去除原詞空白 / Trim whitespace from original word
		...
		// 去除對應詞空白 / Trim whitespace from mapped word
		data[1] = this._trim(data[1]);

		// 若原詞已存在且設定跳過，則直接返回 / Return if original word exists and skip is enabled
	...
	{
		// 若原詞與對應詞相同，則不加入 / Do not add if original and mapped words are the same
		if (n1 !== n2)
		{
			this.TABLE[n1] = n2;
		}

		// 移除反向對應以避免循環 / Remove reverse mapping to avoid cycles
		if (this.TABLE[n2] === n1)
		{
